...
function initMap() {
    ...
    //strip the newline at the end out of our "valid" points list 
    var numberOfCoordinatesInFile = points.length -1;
    var markerCount = Math.floor( numberOfCoordinatesInFile/2 );
	...
	while( numberOfCoordinates < numberOfCoordinatesInFile ){
		...
		if( markerCount === 0 ){
			...
			//center the map at our latest point/location update
			map.center = { lat: Number( points[numberOfCoordinates+1] ), lng: Number( points[numberOfCoordinates] ) };
		}
	...
	//location coordinates are stored in pairs so we have to increment by 2 
